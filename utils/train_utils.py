...
def simclr_train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        mel_spectogram = batch['mel_spectogram']
        mel_spectogram_aug = batch['mel_spectogram_aug']

        b, c, h, w = mel_spectogram.size()
        
        # Concatenate the two inputs into a new dimension
        input_ = torch.cat([mel_spectogram.unsqueeze(1), mel_spectogram_aug.unsqueeze(1)], dim=1)
        
         # Stack the two inputs in the batch dimension (2b, c, d, h, w)
        input_ = input_.view(-1, c, h, w) 
        input_ = input_.cuda(non_blocking=True)

        # Re-do the view from (2b, out) to (b, 2, out) 
