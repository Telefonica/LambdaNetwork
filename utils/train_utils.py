...
def supervised_train(train_loader, model, criterion, optimizer, setup):
    model.train()
    total_loss = 0
...
def supervised_val(val_loader, model, criterion, optimizer, setup):
    model.train()
    total_loss = 0
...
@torch.no_grad()
def supervised_test(val_loader, model, criterion, setup):
    model.eval()
    corrects = 0
    total_samples = 0
