...
for i, path in enumerate(paths):
    ...
    fnr = np.load(os.path.join(path, 'fnr_det.npy'))
    auc = np.load(os.path.join(path, 'auc_det.npy'))

    plt.plot(fpr, fnr, label='{} (AUC: {:0.3f}e-03)'.format(models[i], auc*1000), color=colors[i], linestyle=linestyles[i], linewidth=1.5)
